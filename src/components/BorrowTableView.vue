...
<template>
  <div style="display: flex;justify-content: space-between; align-items: center; height: 90%; background: transparent">
    <!-- 借阅表格 -->
    <a-spin :spinning="loading">
      <a-table :data-source="borrowData" :columns="columns" :row-key="'id'" :pagination="false"
               :row-selection="rowSelection" :scroll="{ y: '70vh' }" :style="{width: '70vw'}" bordered>
        <!--表头自定义，修改为标签-->
    ...
    <!-- 统计信息 -->
    <div
      style="display: flex; justify-content: center; align-items: center;flex-direction: column;width: 30vw;margin-right: 20px;margin-left: 20px">
      <!-- 圆形进度条 -->
      <a-progress :percent="(returnedCount*100/totalCount).toFixed(2)" type="circle" :size="300" :strokeWidth="20" />
      <!-- 统计信息条 -->
